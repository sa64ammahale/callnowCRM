...
if ($segments[0] === 'settings') {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        $res = $link->query("SELECT setting_key, setting_value, description FROM api_settings ORDER BY setting_key");
        $settings = [];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($input['settings'] as $key => $value) {
            ...
            $stmt = $link->prepare("INSERT INTO api_settings (setting_key, setting_value, description) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), description = COALESCE(VALUES(description), description)");
...
elseif ($segments[0] === 'tokens') {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        // Total count
        $countSql = "SELECT COUNT(*) as total FROM api_tokens t $where";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $link->prepare("SELECT ID, NAME, ROLE, STATUS FROM users WHERE ID = ? AND STATUS = 'Active'");
        ...
        // Log
        logActivity($link, $tokenData['user_id'] ?? 0, 'API_TOKEN_CREATED', "Created API token for user #$userId ($user[NAME])", (string)$tokenId, 'api_tokens');
    ...
    if (count($segments) > 1 && is_numeric($segments[1])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
            ...
            logActivity($link, $tokenData['user_id'] ?? 0, 'API_TOKEN_REVOKED', "Revoked API token #$tokenId", (string)$tokenId, 'api_tokens');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
            ...
            if ($updates) {
                $params[] = $tokenId;
                $types .= 'i';
                $stmt = $link->prepare("UPDATE api_tokens SET " . implode(', ', $updates) . " WHERE id = ?");
...
elseif ($segments[0] === 'assignments') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Check user exists
        $stmt = $link->prepare("SELECT ID FROM users WHERE ID = ? AND STATUS = 'Active'");
        $stmt->bind_param('i', $userId);
        $stmt->execute();
        ...
        if ($teamId) {
            $stmt = $link->prepare("SELECT ID FROM teams WHERE ID = ?");
        ...
        logActivity($link, $tokenData['user_id'] ?? 0, 'API_ASSIGNMENT_CREATED', "Assigned $databaseType to user #$userId", '', 'api_database_assignments');
    ...
    if (count($segments) > 1 && is_numeric($segments[1])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
            $stmt = $link->prepare("DELETE FROM api_database_assignments WHERE id = ?");
