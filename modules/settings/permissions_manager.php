...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'save_role_perms') {
            $role = $_POST['role'] ?? '';
            $cs = $link->prepare("SELECT role_name FROM roles WHERE role_name = ?");
            ...
            if (!$cs->get_result()->fetch_assoc()) {
                ...
                foreach (array_keys(getPermissionSeed()) as $pk) {
                ...
                $msg_type = 'success';
                logActivity($link, USER_ID, 'UPDATE', "Updated role permissions for $role");
            }
            $selectedRole = $role;
        }
        ...
        if ($action === 'save_user_perms') {
            ...
            $role = $_POST['role'] ?? '';
            $us = $link->prepare("SELECT ID FROM users WHERE ID = ?");
            $us->bind_param('i', $uid);
            $us->execute();
            if (!$us->get_result()->fetch_assoc()) {
                ...
                $stmt = $link->prepare("INSERT INTO user_permissions (user_id, permission_key, permission_value) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE permission_value = VALUES(permission_value)");
                foreach (array_keys(getPermissionSeed()) as $pk) {
        ...
        if ($action === 'clear_user_perms') {
            $uid = (int)($_POST['user_id'] ?? 0);
            $link->query("DELETE FROM user_permissions WHERE user_id = $uid");
            clearRbacCache();
            $msg = "Overrides cleared; user #$uid now inherits role permissions.";
            $msg_type = 'success';
            $selectedUser = $uid;
        }
        ...
        if ($action === 'assign_role') {
            $uid = (int)($_POST['user_id'] ?? 0);
            $role = $_POST['role'] ?? '';
            $cs = $link->prepare("SELECT role_name, id FROM roles WHERE role_name = ?");
            $cs->bind_param('s', $role);
            $cs->execute();
            ...
            if (!$cr) {
                ...
                $up = $link->prepare("UPDATE users SET ROLE = ?, role_id = ? WHERE ID = ?");
        ...
        if ($action === 'add_feature') {
            ...
            if (!preg_match('/^[a-z][a-z0-9_]*$/', $pkey)) {
                ...
                $stmt = $link->prepare("INSERT INTO permissions (permission_key, label, category, description, is_system) VALUES (?, ?, ?, ?, 0)");
                $stmt->bind_param('ssss', $pkey, $plabel, $pcat, $pdesc);
                $stmt->execute();
                // Backfill a row for every existing role (default 0)
                $ins = $link->prepare("INSERT IGNORE INTO role_permissions (role, permission_key, permission_value) VALUES (?, ?, 0)");
        ...
        if ($action === 'edit_feature') {
            ...
            $pcat = trim($_POST['perm_category'] ?? 'General');
            $pdesc = trim($_POST['perm_description'] ?? '');
            $cs = $link->prepare("SELECT permission_key, is_system FROM permissions WHERE id = ?");
            ...
            if (!$cr) {
                ...
                $s = $link->prepare("UPDATE permissions SET label = ?, category = ?, description = ? WHERE id = ?");
        ...
        if ($action === 'delete_feature') {
            ...
            if (!$cr) {
                ...
                $dp = $link->prepare("DELETE FROM permissions WHERE id = ?");
                $dp->bind_param('i', $id);
                $dp->execute();
                clearRbacCache();
...
$allPermissions = getAllPermissions($link);
$permsByCat = getPermissionsByCategory($link);
$seedKeys = array_keys(getPermissionSeed());

// Default selected role = first role
if ($selectedRole === null && !empty($allRoles)) {
    $selectedRole = $allRoles[0]['role_name'];
}
$rolePerms = [];
if ($selectedRole) {
    $rp = mysqli_query($link, "SELECT permission_key, permission_value FROM role_permissions WHERE role = '" . mysqli_real_escape_string($link, $selectedRole) . "'");
    while ($rp && $row = mysqli_fetch_assoc($rp)) $rolePerms[$row['permission_key']] = (int)$row['permission_value'];
}

// Users for access tab
$allUsers = [];
$ur = mysqli_query($link, "SELECT ID, NAME, ROLE FROM users ORDER BY NAME");
if ($ur) while ($u = mysqli_fetch_assoc($ur)) $allUsers[] = $u;

if ($selectedUser === null && !empty($allUsers)) {
    $selectedUser = (int)$allUsers[0]['ID'];
}
$userInfo = null;
$userPerms = [];
$effectivePerms = [];
if ($selectedUser) {
    $us = $link->prepare("SELECT ID, NAME, ROLE FROM users WHERE ID = ?");
    $us->bind_param('i', $selectedUser);
    $us->execute();
    $userInfo = $us->get_result()->fetch_assoc();
    $up = mysqli_query($link, "SELECT permission_key, permission_value FROM user_permissions WHERE user_id = $selectedUser");
    ...
    if ($userInfo) {
        $rp = mysqli_query($link, "SELECT permission_key, permission_value FROM role_permissions WHERE role = '" . mysqli_real_escape_string($link, $userInfo['ROLE']) . "'");
        $userRolePerms = [];
...
<div class="container page-wrapper">
    ...
    <div class="pm-header">
        ...
        <p>Manage permission keys, roles, role permissions, and per-user access</p>
    </div>
    ...
    <div class="pm-tabs">
        ...
        <a href="<?= url('modules/settings/permissions_manager.php') ?>?tab=roles" class="pm-tab <?= $tab==='roles'?'active':'' ?>"><i class="bi bi-diagram-3"></i> Roles</a>
        <a href="<?= url('modules/settings/permissions_manager.php') ?>?tab=role_perms" class="pm-tab <?= $tab==='role_perms'?'active':'' ?>"><i class="bi bi-shield-check"></i> Role Permissions</a>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end" style="white-space:nowrap;">
                                ...
                                <form method="POST" style="display:inline;" onsubmit="return confirm('Delete feature &quot;<?= htmlspecialchars($p['permission_key']) ?>&quot;? This removes it for all roles.')">
        ...
        <div class="pm-card-body">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <div>
                    ...
                    <p style="font-size:0.75rem;color:var(--st-ink-soft);margin:0;">Create roles. New custom roles start with <strong>all permissions OFF</strong> (secure by default).</p>
                </div>
    ...
    <div class="pm-card">
        <div class="pm-card-body">
            <h5 style="font-size:1rem;font-weight:700;color:var(--st-ink);margin:0 0 0.2rem 0;"><i class="bi bi-shield-check"></i> Role Permissions</h5>
            <p style="font-size:0.75rem;color:var(--st-ink-soft);margin:0 0 1rem 0;">Select a role, then toggle its permissions. <strong>Admin</strong> always has full access.</p>
            ...
            <form method="POST">
                ...
                <div class="text-end">
                    <button type="submit" class="pm-btn-primary"><i class="bi bi-check-lg"></i> Save Permissions</button>
                </div>
                <?php endif; ?>
            </form>
            <?php endif; ?>
                    ...
                    <div class="modal-content" style="border-radius:0.75rem;border:1px solid var(--st-border);">
                        <form method="POST">
                            ...
                            <div class="modal-body text-center" style="padding:1.5rem;">
                                ...
                                <p style="margin:0.75rem 0;font-size:0.8125rem;">Remove all per-user overrides for <strong><?= htmlspecialchars($userInfo['NAME']) ?></strong>? They will inherit the role's permissions.</p>
                            </div>
