...
$lead = mysqli_fetch_assoc(mysqli_query($link, "SELECT lead_id, assigned_to, team_id, sent_backward_flag FROM " . tn('TBL_LEADS') . " WHERE lead_id = $leadId"));
...
$link->query("UPDATE " . tn('TBL_LEADS') . " SET sent_backward_flag = $newVal, updated_at = NOW() WHERE lead_id = $leadId");
