...
$stmt = mysqli_prepare($link, "SELECT lead_id, assigned_to, team_id, sent_backward_flag FROM " . tn('TBL_LEADS') . " WHERE lead_id = ? LIMIT 1");
...
$upd = mysqli_prepare($link, "UPDATE " . tn('TBL_LEADS') . " SET sent_backward_flag = ?, updated_at = NOW() WHERE lead_id = ?");
