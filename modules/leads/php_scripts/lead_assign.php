...
$lead = mysqli_fetch_assoc(mysqli_query($link, "SELECT lead_id, assigned_to, team_id, assigned_by FROM TBL_LEADS WHERE lead_id = $leadId"));
if (!$lead) {
...
$realToTeam = $toTeamId;
if ($toUserId > 0) {
    $u = mysqli_fetch_assoc(mysqli_query($link, "SELECT ID, TEAM_ID FROM TBL_USERS WHERE ID = $toUserId"));
    if (!$u) {
...
$stmt = mysqli_prepare($link, "UPDATE TBL_LEADS SET assigned_to = ?, team_id = ?, assigned_by = ?, updated_at = NOW() WHERE lead_id = ?");
mysqli_stmt_bind_param($stmt, 'iiii', $realToUser, $realToTeam, USER_ID, $leadId);
if (!mysqli_stmt_execute($stmt)) {
    api_error('Failed to assign lead', 500);
}

$ins = mysqli_prepare($link, "INSERT INTO TBL_LEAD_ASSIGNMENTS (lead_id, from_user_id, to_user_id, from_team_id, to_team_id, reason, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
mysqli_stmt_bind_param($ins, 'iiiiis', $leadId, $fromUserId, $realToUser, $fromTeamId, $realToTeam, $reason);
mysqli_stmt_execute($ins);

logActivity($link, USER_ID, 'ASSIGN', "Reassigned lead #$leadId to user #$realToUser" . ($reason ? " ($reason)" : ''), (string)$leadId, 'TBL_LEADS');
