...
$lead = mysqli_fetch_assoc(mysqli_query($link, "SELECT lead_id, assigned_to, team_id, forwarded_flag FROM " . tn('TBL_LEADS') . " WHERE lead_id = $leadId"));
...
$link->query("UPDATE " . tn('TBL_LEADS') . " SET forwarded_flag = $newVal, updated_at = NOW() WHERE lead_id = $leadId");
