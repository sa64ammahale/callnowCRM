...
/* dev error reporting (remove in production) */
ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

/* Temporary debug logging — remove after debugging is done */
file_put_contents('/tmp/upload_debug.log', "=== UPLOAD PAGE LOAD " . date('c') . " ===\n", FILE_APPEND);
file_put_contents('/tmp/upload_debug.log', "_SERVER: " . print_r($_SERVER, true) . "\n", FILE_APPEND);
file_put_contents('/tmp/upload_debug.log', "_POST: " . print_r($_POST, true) . "\n", FILE_APPEND);
file_put_contents('/tmp/upload_debug.log', "_FILES: " . print_r($_FILES, true) . "\n", FILE_APPEND);
file_put_contents('/tmp/upload_debug.log', "upload_max_filesize=" . ini_get('upload_max_filesize') . " post_max_size=" . ini_get('post_max_size') . "\n\n", FILE_APPEND);

/* Defensive auto-enable: if POST with file present but missing 'import', simulate it */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file']) && empty($_POST['import'])) {
    file_put_contents('/tmp/upload_debug.log', date('c') . " Auto-enabling import because file present but import missing\n", FILE_APPEND);
    $_POST['import'] = '1';
}
    ...
    if (!mysqli_query($link, $create_sql)) {
        ...
        if ($ins_stmt) {
            ...
            if (mysqli_stmt_execute($ins_stmt)) {
                ...
                $summaryType = "success";
                file_put_contents('/tmp/upload_debug.log', date('c') . " Archive moved $moved deleted $deleted_total\n", FILE_APPEND);
            } else {
                $summary = "Archive insert failed: " . mysqli_stmt_error($ins_stmt);
                $summaryType = "danger";
                mysqli_stmt_close($ins_stmt);
                file_put_contents('/tmp/upload_debug.log', date('c') . " Archive insert failed: " . mysqli_stmt_error($ins_stmt) . "\n", FILE_APPEND);
            }
        } else {
            $summary = "Archive prepare failed: " . mysqli_error($link);
            $summaryType = "danger";
            file_put_contents('/tmp/upload_debug.log', date('c') . " Archive prepare failed: " . mysqli_error($link) . "\n", FILE_APPEND);
        }
    }
}
...
if (isset($_POST['import']) && isset($_FILES['file']) && isset($_FILES['file']['error']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    file_put_contents('/tmp/upload_debug.log', date('c') . " Inside import branch\n", FILE_APPEND);
    ...
    if (empty($_POST['terms_agreed'])) {
        ...
        $summaryType = "danger";
        file_put_contents('/tmp/upload_debug.log', date('c') . " Terms not accepted\n", FILE_APPEND);
        return;
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024 || $ext !== 'csv') {
        ...
        file_put_contents('/tmp/upload_debug.log', date('c') . " Rejected file due to size/extension\n", FILE_APPEND);
    } else {
        ...
        if (!move_uploaded_file($file['tmp_name'], $path)) {
            ...
            file_put_contents('/tmp/upload_debug.log', date('c') . " move_uploaded_file failed for " . $file['tmp_name'] . "\n", FILE_APPEND);
        } else {
            if (($handle = fopen($path, 'r')) === false) {
                $summary = "Cannot open CSV file for reading.";
                $summaryType = "danger";
                logLine($logs, "fopen() failed for $path.");
                file_put_contents('/tmp/upload_debug.log', date('c') . " fopen failed for $path\n", FILE_APPEND);
            } else {
                 $first = null;
                ...
                if ($has_header) {
                    ...
                    if ($first === false) {
                        ...
                        fclose($handle);
                        @unlink($path);
                        file_put_contents('/tmp/upload_debug.log', date('c') . " CSV empty or unreadable\n", FILE_APPEND);
                        $first = false;
                    } else {
                ...
                if ($first === false && $has_header) {
                    ...
                    if ($has_header) {
                        ...
                        if ($header_index_map['mobile'] === null) {
                            ...
                            fclose($handle);
                            @unlink($path);
                            file_put_contents('/tmp/upload_debug.log', date('c') . " Header validation failed\n", FILE_APPEND);
                            $handle = null;
                        }
                    } else {
                ...
                if ($handle) {
                        ...
                        foreach ($targets as $tkey => $tinfo) {
                            ...
                            if ($dryrun) {
                                ...
                                if (!$stmt) {
                                    ...
                                    file_put_contents('/tmp/upload_debug.log', date('c') . " Prepare failed: " . mysqli_error($link) . "\n", FILE_APPEND);
                                    continue;
                                }
                                if ($bind_types !== '') {
                                    $bind_names = [];
                                ...
                                if (mysqli_stmt_execute($stmt)) {
                                    ...
                                    $stats['invalid']++;
                                    file_put_contents('/tmp/upload_debug.log', date('c') . " Insert failed: $err\n", FILE_APPEND);
                                }
                    ...
                    if ($dryrun) {
                        ...
                        file_put_contents('/tmp/upload_debug.log', date('c') . " DRY-RUN finished\n", FILE_APPEND);
                    } else {
                        ...
                        file_put_contents('/tmp/upload_debug.log', date('c') . " Import summary: " . $summary . "\n", FILE_APPEND);
                    }
                } // end handle valid
            }
        }
    ...
    file_put_contents('/tmp/upload_debug.log', date('c') . " Outside import branch (conditions not met)\n", FILE_APPEND);
}
?>
<!doctype html>
