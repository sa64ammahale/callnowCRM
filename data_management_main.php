...
// Get all telecallers for assign dropdown
$users_result = mysqli_query($link, "SELECT ID, NAME FROM USERS WHERE ROLE = 'telecaller' ORDER BY NAME");
$telecallers = [];
...
$(document).ready(function() {
    ...
    const table = $('#mainTable').DataTable({
        ...
        serverSide: true,
        responsive: true,
        ajax: 'maindatabase_ajax/datatable.php',
        pageLength: 2500,
        ...
        buttons: [
            ...
            { text: '<i class="bi bi-trash3"></i> Delete Selected', className: 'btn btn-danger btn-sm', action: function() { bulkDelete(); }},
            {
                ...
                action: function () {
                    $.get('maindatabase_ajax/get_total_count.php', function (total) {
                        ...
                        const win = window.open('maindatabase_ajax/download_bach.php', '_blank');
                        if (win) {
    ...
    $('#doAssign').on('click', function() {
        ...
        $.post('maindatabase_ajax/bulk_assign.php', { ids: ids, user_id: userId }, function(res) {
            if (res.success) {
                table.ajax.reload();
    ...
    window.bulkDelete = function() {
        ...
        $.post('maindatabase_ajax/bulk_delete.php', { ids: ids }, function(res) {
            if (res.success) {
                table.ajax.reload();
